...
opti = [0, 2, 4, 6, 8, 12, 14, 18, 20, 26, 30, 36, 38, 44, 48, 54]
class SIMON(Cipher):
    ...
    def genConstraints_Additional(self, r):
        ...
        pre = self.genMyObjectiveFun_to_Round(r, r)
        if pre != "": 
            temp = pre + ' - xobj <= ' + str(-opti[r - 2])
            constraints = constraints + [temp]
        pre = self.genMyObjectiveFun_to_Round(1, 1)
        if pre != "": 
            temp = pre + ' - xobj <= ' + str(-opti[r - 2])
            constraints = constraints + [temp]
...
# mySIMON.traceSol("SIMON14.sol", 14)
